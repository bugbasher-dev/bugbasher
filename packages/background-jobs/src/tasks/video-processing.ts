...
async function uploadVideoThumbnail(userId: string, noteId: string, videoId: string, thumbnailBuffer: Buffer, fileExtension: string = 'jpg') {
...
export const videoProcessingTask = task({
    ...
    run: async (payload: {
        ...
        const { videoUrl, videoHeaders, videoId, noteId, organizationId, userId } = payload

        logger.info("Starting video processing", { videoId, noteId, organizationId, userId })
        ...
        const thumbnailPath = path.join(tempDirectory, `thumbnail_${videoId}_${Date.now()}.jpg`)
        const tempVideoPath = path.join(tempDirectory, `video_${videoId}_${Date.now()}.mp4`)
        ...
        try {
            ...
            console.log("Video saved to temp file", { tempVideoPath, size: videoBuffer.length })
            ...
            const thumbnailKey = await uploadVideoThumbnail(userId, noteId, videoId, thumbnailBuffer)
            ...
            try {
                ...
                logger.error("Failed to update video upload status to failed", { dbError })
            }
