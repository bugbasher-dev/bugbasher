...
function getSignedPutRequestInfo(file: { type: string; name: string }, key: string) {
...
async function uploadImageThumbnail(userId: string, noteId: string, imageId: string, thumbnailBuffer: Buffer, fileExtension: string = 'jpg') {
...
function calculateThumbnailDimensions(originalWidth: number, originalHeight: number, maxWidth: number = 400) {
...
export const imageProcessingTask = task({
    ...
    run: async (payload: {
        ...
        const { imageUrl, imageHeaders, imageId, noteId, organizationId, userId } = payload

        logger.info("Starting image processing", { imageId, noteId, organizationId, userId })
        ...
        const thumbnailPath = path.join(tempDirectory, `thumbnail_${imageId}_${Date.now()}.jpg`)
        const tempImagePath = path.join(tempDirectory, `image_${imageId}_${Date.now()}`)
        ...
        try {
            ...
            let metadata: { width?: number; height?: number; format?: string } | undefined
            let jimp: any
            ...
            try {
                ...
                thumbnailBuffer = await resizedImage.quality(80).getBufferAsync(jimp.MIME_JPEG)
            ...
            const thumbnailKey = await uploadImageThumbnail(userId, noteId, imageId, thumbnailBuffer)
            ...
            try {
                ...
                logger.error("Failed to update image upload status to failed", { dbError })
            }
